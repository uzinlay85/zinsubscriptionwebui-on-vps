...
async def check_inbounds():
    db = SessionLocal()
    try:
        # Get the first 3x-ui server
        server = db.query(Server).filter(Server.type == "3x-ui").first()
        if not server:
            ...
            async with session.get(
                ...
                if resp.status == 200:
                    ...
                    if data.get("success"):
                        ...
                        print(f"\nFound {len(inbounds)} inbounds on 3x-ui panel:")
                    ...
                    print(f"Failed to fetch inbounds. Status: {resp.status}")
    finally:
        db.close()
