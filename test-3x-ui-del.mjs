...
async function run() {
  ...
  const loginRes = await fetch(`${cleanUrl}/login`, {
    ...
    body: new URLSearchParams({
      username: server.username || server.auth_username,
      password: server.password || server.auth_password,
    })
  });
  ...
  let inboundData;
  try {
    inboundData = JSON.parse(responseText);
  } catch(e) {
    console.log("Failed to parse inbound data:", responseText.substring(0, 200));
    return;
  }
  ...
  if (clients.length > 0) {
    ...
    console.log('Client details:', testClient.email, testClient.id, testClient.password);
