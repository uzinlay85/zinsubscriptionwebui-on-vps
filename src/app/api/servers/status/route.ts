...
function pingServer(apiUrl: string): Promise<{ online: boolean; latency?: number }> {
  return new Promise((resolve) => {
    try {
      ...
      // Outline and 3x-ui servers commonly use self-signed TLS certificates.
      // We must bypass cert validation for ping checks to work correctly.
      // This is a connectivity check only — no sensitive data is transmitted.
      const options = {
        ...
        rejectUnauthorized: false, // Required: VPN panel servers use self-signed certs
      };
...
export async function GET() {
  try {
    // Note: Auth is already enforced by middleware.ts for all non-public routes.
    // Adding requireAuth() here caused issues in Next.js 16 route handler context.
